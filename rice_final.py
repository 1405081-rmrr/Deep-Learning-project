...
image_folder_init = "./Dataset/SYMPTOM_Resized"
image_folder = "./Dataset/Resized_Original_Data"
...
boundary = int(len(dataset)*0.8) #80% training
train_dataset = dataset.take(boundary)
test_dataset = dataset.skip(boundary)
boundary2 = int(len(dataset)*0.1)  #10% validation
...
model = models.Sequential([
    ...
    layers.Conv2D(24, (3, 3), activation='relu'), #24 filters size 3 X 3
    layers.BatchNormalization(),
    layers.MaxPooling2D((2, 2)),
...
accuracy_history = history1.history["val_accuracy"]
numpy_accuracy_history = np.array(accuracy_history)
np.savetxt("accuracy5.txt", numpy_accuracy_history, delimiter=",")


for images_batch, labels_batch in test_dataset_final.take(6): # First Image from 5 Batches
